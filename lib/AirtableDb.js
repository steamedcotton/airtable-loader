      ...
      await asyncForEach(fields, async ({ mapToName, name, resolve }) => {
        if (resolve) {
          ...
          if (_.isEmpty(recordKeys)) {
            ...
            console.log(
              ERROR_PREFIX,
              chalk.red(`The field [${name}] should be an array of ids, instead it is a ${typeof recordKeys}`)
            );
          } else {
            const records = await Promise.all(recordKeys).map((id) => this.selectSingleRecord(id, resolve), {
              concurrency: 1
            });
